...
// 반복문으로 DFS(last in, last out) 구현     
public class B2178_v5 {
	...
	static int search(char[][] map) {
		...
		Deque<Node> queue = new ArrayDeque<>(); // 방문할 칸 목록
		queue.add(new Node(0, 0, 1));           // 시작 위치
		while (queue.size() > 0) {
			Node current = queue.removeLast();         // 현재 위치
			...
			for (int[] d : D) {
				...
				queue.addLast(new Node(r1, c1, distance + 1));
			}
		}
